...
export class ScannedComponent implements OnInit, OnDestroy {
  ...
  executecall(params: Paramlist): void {
    ...
    this.filters = {
      ...
      AcctMonth: params.accountingmonthval ? params.accountingmonthval.toString() : new Date().getMonth().toString(),
  ...
  //  { headerName: 'Bill Currency', field: 'billcurrency', width: 110 },
    ...
    { headerName: 'Remit ID', field: 'aP_Remit_id', width: 110 },
    { headerName: 'Vendor Number', field: 'vendor_code', width: 110 },
  ...
  loadData(filters: BillsTParams): void {
    ...
    this.billsSubscription = this.billsService.getBillsReport(filters).subscribe({
      ...
      error: (error) => {
        console.error('Error loading Scanned Bills:', error);
