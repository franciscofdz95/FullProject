...
namespace App.Files
{
    ...
    public class TIF
    {
        ...
        private int GetPageCount()
        {
            ...
            try
            {
                Img = Image.FromFile(this.mFilePathName);
        ...
        {
            if ((PageNum < 1) || (PageNum > this.mPageCount))
            {
            ...
            try
            {
                SrcImg = Image.FromFile(this.mFilePathName);
                ms = new MemoryStream();
        ...
        public TIFPageCollection GetTiffImages(int StartPageNum, int EndPageNum)
        {
            ...
            if (((StartPageNum < 1) || (EndPageNum > this.mPageCount)) || (EndPageNum > StartPageNum))
            {
        ...
        public Image this[int PageNum]
        {
            get
            {
                ...
                try
                {
                    this.mImg = this.GetTiffImage(PageNum);
                    TiffPage = this.mImg;
                }
                catch (Exception ex)
                {
